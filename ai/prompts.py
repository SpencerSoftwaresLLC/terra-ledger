...
def build_help_system_prompt(language="en"):
...
- Answer in Spanish unless the user clearly asks for English.
...
Important response language behavior:
- Respond in Spanish by default.
- Keep TerraLedger page and module names in their real app naming when needed.
- When helpful, you may explain an English button or page name in Spanish rather than inventing a translated page name that does not exist in the app.
""".strip()
...
def build_page_context(page_title=None, route=None, company_name=None, user_name=None, user_role=None, language="en"):
    language = str(language or "en").strip().lower()
    is_es = language == "es"
    ...
    if company_name:
        parts.append(f"{'Company' if not is_es else 'Empresa'}: {company_name}")
    if user_name:
        parts.append(f"{'User' if not is_es else 'Usuario'}: {user_name}")
    if user_role:
        parts.append(f"{'Role' if not is_es else 'Rol'}: {user_role}")
    if page_title:
        parts.append(f"{'Current page' if not is_es else 'Página actual'}: {page_title}")
    if route:
        parts.append(f"{'Current route' if not is_es else 'Ruta actual'}: {route}")

    if not parts:
        return "No page context provided." if not is_es else "No se proporcionó contexto de la página."

    return "\n".join(parts)


def build_help_input_messages(user_question, page_context_text, prior_messages=None, language="en"):
    language = str(language or "en").strip().lower()
    is_es = language == "es"

    if is_es:
        primary_instruction = (
            "Answer as TerraLedger support in Spanish. "
        ...
        page_context_label = "Page context"
    else:
        primary_instruction = (
    ...
    messages = [
        {
            "role": "developer",
            "content": primary_instruction
        },
        {
            "role": "developer",
            "content": f"{page_context_label}:\n{page_context_text}"
        }
    ]
